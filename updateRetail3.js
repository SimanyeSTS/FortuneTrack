...
(async () => {
  ...
  try {
    ...
    await Retail.updateRetailData3();

    // Log success with time taken
    const timeTaken = ((Date.now() - startTime) / 1000).toFixed(2); // Time in seconds
    console.log(`✅ Retail Data-3 updated successfully in ${timeTaken} seconds.`);

    process.exit(0)

  } catch (err) {
    // Enhanced error handling
    console.error('❌ An error occurred during the Retail Data-3 Update Process.');
    ...
    console.log('Retail Data-3 update process finished.');
  }
})();
